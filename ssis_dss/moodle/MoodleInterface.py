...
# moodle stuff:
from ssis_dss.moodle import MoodleDBSchema    # yes, import the module itself, used for getattr statements
from ssis_dss.moodle.MoodleDBSchema import *  # and, yes, import all the terms we need to refer to the tables as classes

# app stuff:
from ssis_dss.utils import Namespace, time_now
try:
    import ssis_dss_settings
except ImportError:
    pass # Happens when run as __main__
    ...
    def update_table(self, table, where={}, **kwargs):
        ...
        with self.db_session() as session:
            try:
                ...
                return {'result': False, 'message': "MDB.update_table: Cannot update {} with this query: {} because NOT found".format(table_class, where)}
            except MultipleResultsFound:
                return {'result': False, 'message': "MDB.update_table: Cannot update {} with this idnumber: {} because multiple results found".format(table_class, where)}
            except sqlalchemy.exc.IntegrityError:
                return {'result': False, 'message': "MDB.update_table: Cannot update {} with this idnumber: {} because of an integrity error (it has to be unique but there is already someone in there)".format(table_class, where)}
            for key in kwargs.keys():
                setattr(instance, key, kwargs[key])
            session.add(instance)
        return {'result': True}
...
class MoodleInterface(MoodleInter):
    ...
    TEACHING_LEARNING_CATEGORY_ID = ssis_dss_settings['SSIS_DB']['db_tl_cat_id']
